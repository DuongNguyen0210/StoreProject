...
void MainWindow::onAddSanPham(const QModelIndex &index)
{
    ...
    bool ok;
    int quantityToAdd = QInputDialog::getInt(this, "Nhập số lượng", QString("Nhập số lượng cho %1:").arg(p->getName()), 1, 1, maxStock, 1, &ok);
...
void MainWindow::finalizeThanhToan(const QString& paymentMethod)
{
    ...
    if (currentBill->getTierDiscountPercent() == 0.0)
    {
        if (c != nullptr)
        {
            double tierDiscount = c->getTierDiscountPercent();
