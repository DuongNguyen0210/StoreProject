...
Store::Store(const QString& name) : name(name), totalRevenue(0.0), productByName(101), productById(101),
    customerByName(101), customerById(101), customerByPhone(101), userByName(101), userById(101)
{}

Store::~Store()
{
    productById.forEach( [](const QString&, Product* p) {delete p;} );
    customerById.forEach( [](const QString&, Customer* c) {delete c;} );
    userById.forEach( [](const QString&, User* u) {delete u;} );
